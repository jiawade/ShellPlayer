...
const FavoritesScreen: React.FC = () => {
  ...
  const { tracks, currentTrack, favoriteIds, repeatMode, hideDuplicates } = useAppSelector(
    s => s.music,
  );
    ...
    return (
      <View style={[styles.empty, { backgroundColor: colors.bg }]}>
        <View style={[styles.emptyIcon, { backgroundColor: colors.bgCard }]}>
          <Icon name="heart-outline" size={72} color={colors.heartDim} />
        </View>
        <Text style={{ fontSize: sizes.xl, fontWeight: '600', color: colors.textSecondary }}>
          {t('favorites.empty.noFavorites')}
        </Text>
        <Text
          style={{
            fontSize: sizes.md,
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      <View style={styles.header}>
        ...
        <View style={[styles.badge, { backgroundColor: colors.heartDim }]}>
          <Icon name="heart" size={14} color={colors.heart} />
          <Text style={{ fontSize: sizes.sm, color: colors.heart, fontWeight: '600' }}>
            {favTracks.length}
          </Text>
        </View>
      </View>
