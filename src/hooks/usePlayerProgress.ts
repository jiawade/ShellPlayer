...
export function usePlayerSync() {
    ...
    const lastNPTrackId = useRef<string | undefined>(undefined);
    useEffect(() => {
        if (Platform.OS !== 'ios') return;
        const playing = playbackState.state === State.Playing;
        if (!playing || position < 0.5) return;

        const trackChanged = activeTrack?.id !== lastNPTrackId.current;
        if (trackChanged) {
            // Track just changed – force immediate sync with duration so the lock
            // screen progress bar shows correct values instead of "--".
            lastNPTrackId.current = activeTrack?.id;
            lastNPSync.current = Date.now();
            if (duration > 0) {
                TrackPlayer.updateNowPlayingMetadata({ elapsedTime: position, duration }).catch(() => {});
            }
            return;
        }
    ...
    }, [position, playbackState.state, activeTrack?.id, duration]);
