        ...
        <View style={styles.cardRow}>
          {[
            ...
            <View
              ...
              <Text style={[styles.cardLabel, { color: colors.textSecondary, fontSize: sizes.sm }]}>
                {item.label}
              </Text>
            </View>
          ))}
        ...
        {topSongs.length > 0 && (
          <View
            ...
            <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>
              {t('statistics.topSongs')}
            </Text>
            {topSongs.map((song, i) => (
              <View key={song.id} style={[styles.rankRow, { borderBottomColor: colors.border }]}>
                <Text style={[styles.rankNum, { color: i < 3 ? colors.accent : colors.textMuted }]}>
                  {i + 1}
                </Text>
                <View style={styles.rankInfo}>
                  ...
                  <Text style={[styles.rankSub, { color: colors.textSecondary }]} numberOfLines={1}>
                    {song.artist}
                  </Text>
                </View>
                <Text style={[styles.rankCount, { color: colors.textMuted }]}>
                  {song.count} {t('statistics.plays')}
                </Text>
              </View>
            ))}
          </View>
              ...
              <View style={styles.heatmapLabels}>
                {DAY_LABELS.map(label => (
                  <View key={label} style={styles.heatmapLabelCell}>
                    <Text style={[styles.heatmapLabelText, { color: colors.textMuted }]}>
                      {label}
                    </Text>
                  </View>
                ))}
          ...
          {formatStats.items.length > 0 ? (
            <View style={styles.formatList}>
              {formatStats.items.map(([ext, count], i) => {
                const pct =
                  formatStats.total > 0 ? ((count / formatStats.total) * 100).toFixed(1) : '0';
                  ...
                  <View key={ext} style={styles.formatRow}>
                    ...
                    <View style={[styles.formatBarBg, { backgroundColor: colors.bg }]}>
                      <View
                        style={[
                          styles.formatBar,
                          { width: `${barRatio * 100}%`, backgroundColor: barColor },
                        ]}
                      />
                    </View>
                  </View>
