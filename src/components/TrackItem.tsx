...
const TrackItem: React.FC<Props> = ({ track, isActive, onPress, onToggleFavorite, onOpenMenu, batchMode, batchSelected }) => {
  ...
  return (
    <TouchableOpacity
      style={[styles.row, { borderRadius: sizes.radius },
      ...
      <View style={styles.coverWrap}>
        ...
        {isActive && (
          <View style={styles.bars}>
            <View style={[styles.bar, { height: 10, backgroundColor: colors.accent }]} />
            <View style={[styles.bar, { height: 14, backgroundColor: colors.accent }]} />
            <View style={[styles.bar, { height: 8, backgroundColor: colors.accent }]} />
          </View>
        )}
      </View>

      <View style={styles.info}>
        <Text style={[{ fontSize: sizes.lg, color: colors.textPrimary, fontWeight: '600', marginBottom: 3 }, isActive && { color: colors.accent }]} numberOfLines={1}>{track.title}</Text>
        <Text style={{ fontSize: sizes.sm, color: colors.textSecondary }} numberOfLines={1}>{track.artist}</Text>
      </View>
...
const styles = StyleSheet.create({
  row: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10, marginHorizontal: 12, marginVertical: 3 },
